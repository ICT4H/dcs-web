...
def take_psql_dump(backup_path, name_prefix):
    ...
    sudo('pg_dump mangrove | gzip > %s/%s' % (backup_path, backup_name), user=MANGROVER_USER)
...
def take_couchdbmain_dump(couchdb_path, backup_path, name_prefix):
    ...
    with cd(couchdb_path):
        sudo('tar -czvPf  %s/%s  couchdbmain' % (backup_path, backup), user=COUCH_USER)
...
def take_couchdbfeed_dump(couchdb_path, backup_path, name_prefix):
    ...
    with cd(couchdb_path):
        sudo('tar -czvPf  %s/%s couchdbfeed' % (backup_path, backup_name), user=COUCH_USER)
...
def take_elastic_search_index_dump(backup_path, name_prefix):
    ...
    with cd('/var/lib/elasticsearch'):
        ...
        sudo('tar -czvPf %s/%s %s' % (backup_path, backup_name, index_files[0]), user=ELASTIC_USER)
...
def take_database_backup(backup=False):
    if backup == 'true':
        ...
        backup_path = "/tmp/db_backup"+backup_prefix
        run('mkdir -p %s'%backup_path)
        sudo('chmod 777 %s'%backup_path, user=env.user)
...
def apply_elasticsearch_backup(backup_dir, index_bkp_file):
    ...
    with cd('/var/lib/elasticsearch'), settings(sudo_user=ELASTIC_USER):
...
def apply_backup(backup_dir):
    ...
    couchdb_path = '/opt/apache-couchdb/var/lib'
    apply_couchdb_backup(backup_dir, couchdb_path, couchdbmain_bkp_file, 'couchdbmain')
    couchdbfeed_bkp_file = get_backup_file(backup_dir, 'mangrove_couchdb_feed')
    apply_couchdb_backup(backup_dir, couchdb_path, couchdbfeed_bkp_file, 'couchdbfeed')
